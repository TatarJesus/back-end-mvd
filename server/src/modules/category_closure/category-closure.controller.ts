...
export class CategoryClosureController {
  ...
  @Get(':ancestor_id')
  findAllSubcategory(@Param('ancestor_id') ancestor_id: string) {
    return this.categoryClosureService.findAllSubcategory(+ancestor_id);
  }

  @Put('/:ancestor_id/:descendant_id')
  update(
    @Param('ancestor_id') ancestor_id: string,
    @Param('descendant_id') descendant_id: string,
